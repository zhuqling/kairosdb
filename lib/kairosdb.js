...
KairosDB.prototype.buildStub = function () {
  ...
  this.endpoints.forEach(function (endpoint) {
    ...
    self[name] = function (args) {
      ...
      return self.execute(url, {method: method}, args);
    };
  });
};

KairosDB.prototype.execute = function (endpoint, options, payload) {
  var self = this;
    ...
    try {
      ...
      request(_options, function (error, res, body) {
        ...
        if (res.statusCode === 404)
          return reject(new Error('404: page not found'));
        else if (res.statusCode === 400)
          return reject(new Error('400: server error'));
